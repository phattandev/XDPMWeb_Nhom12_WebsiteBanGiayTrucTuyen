            ...
            <div class="xl:col-span-2">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="px-6 md:px-8 py-6 border-b border-slate-100 bg-linear-to-r from-slate-900 to-slate-800">
                        ...
                        <p class="text-slate-300 mt-2">Thông tin cơ bản đã được điền sẵn từ tài khoản của bạn. Nếu muốn giao tới địa chỉ khác, bạn chỉ cần chỉnh lại trước khi xác nhận.</p>
                    </div>
                    ...
                    <form action="{{ route('checkout.process') }}" method="POST" class="p-6 md:p-8 space-y-8">
                        ...
                        @if(blank($user->phone) || blank($user->address))
                            <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-800">
                                ...
                                <p class="text-sm mt-1">Bạn vẫn có thể tiếp tục bằng cách điền tay bên dưới, hoặc cập nhật tại trang hồ sơ để lần sau nhanh hơn.</p>
                        ...
                        <section class="space-y-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input
                                        ...
                                        name="ten_nguoi_nhan"
                                        value="{{ $prefillName }}"
                                        class="w-full rounded-xl border-slate-300 focus:ring-orange-500 focus:border-orange-500 py-3 px-4"
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    required
                                >{{ $prefillAddress }}</textarea>
                                <p class="text-xs text-slate-500 mt-2">Hệ thống đang lấy địa chỉ mặc định từ tài khoản của bạn. Bạn có thể sửa tại đây nếu muốn giao đến nơi khác.</p>
                            </div>
                        </section>
                            ...
                            @foreach($cart as $item)
                                <div class="flex gap-4">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="font-semibold text-slate-900 line-clamp-2">{{ $item['name'] }}</p>
                                        <p class="text-sm text-slate-500 mt-1">Size {{ $item['size'] }} | {{ $item['color'] }}</p>
                                        <div class="flex items-center justify-between mt-2 text-sm">
                                            <span class="text-slate-500">x{{ $item['quantity'] }}</span>
                                            <span class="font-bold text-orange-600">{{ number_format($item['price'] * $item['quantity'], 0, ',', '.') }} ₫</span>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
